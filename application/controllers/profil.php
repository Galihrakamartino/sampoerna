...
class Profil extends CI_Controller {
	...
	function index() {
		$data['pegawai'] = $this->UM->getProfil($this->session->userdata('logged_in')['id_pegawai']);
	...
	function editprofil()
	{
		...
		$this->form_validation->set_rules('passwordlama', 'Password Lama', 'trim|callback_cekPasswordLama');
		$this->form_validation->set_rules('passwordbaru', 'Password Baru', 'trim|callback_cekTerisi');
		$this->form_validation->set_rules('konfirmpassword', 'Konfirmasi Password Baru', 'trim|matches[passwordbaru]');
		$data['pegawai2'] = $this->UM->getProfil($this->session->userdata('logged_in')['id_pegawai']);
		...
		$this->load->view('menu');
			...
			$this->UM->updateProfil($this->session->userdata('logged_in')['id_pegawai']);
		...
		$this->form_validation->set_rules('idpegawai', 'Id Pegawai', 'trim|required');
            if ($this->form_validation->run()==FALSE) {
                ...
                if ( ! $this->upload->do_upload('userfile')){
                    ...
                    $this->UM->editFoto($this->session->userdata('logged_in')['id_pegawai']);
                    redirect('profil/','refresh');
                }
            }
